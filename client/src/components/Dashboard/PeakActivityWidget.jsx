...
const PeakActivityWidget = () => {
  ...
  if (isLoading) {
    return (
      <div
        dir="rtl"
        className="frosted-glass-card p-6 rounded-2xl flex flex-col h-full w-full animate-pulse"
      >
  ...
  if (error) {
    return (
      <div
        dir="rtl"
        className="frosted-glass-card p-6 rounded-2xl flex flex-col items-center justify-center h-full w-full min-h-[300px]"
      >
        <span className="material-icons-round text-red-200 text-4xl mb-2">
          error_outline
        </span>
  ...
  const CustomTooltip = ({active, payload, label}) => {
    if (active && payload && payload.length) {
      return (
        <div
          className="bg-white dark:bg-zinc-800 p-3 rounded-lg shadow-lg border border-zinc-100 dark:border-zinc-700 text-sm"
          dir="rtl"
        >
  ...
  const CustomBar = (props) => {
    ...
    return (
      <g>
        ...
        />
        {isToday && (
          <circle cx={x + width / 2} cy={y - 8} r={4} fill="#3b82f6" />
        )}
      </g>
    );
  };
  ...
  return (
    <div
      dir="rtl"
      className="frosted-glass-card p-6 rounded-2xl flex flex-col h-full w-full min-h-[300px]"
    >
      <div className="flex items-center justify-between mb-6">
        <div>
              ...
              {chartData.map((entry, index) => {
                ...
                return (
                  <Cell
                    ...
                    fill={fill}
                    className={
                      !isPeak && !isToday ? "dark:fill-green-500/20" : ""
                    }
                  />
                );
              })}
