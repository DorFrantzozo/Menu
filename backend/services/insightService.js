...
export const processAllProRestaurants = async () => {
    try {
        ...
        const tasks = proUsers.map(user => limit(() => processSingleRestaurant(user._id)));
        const results = await Promise.allSettled(tasks);

        const failed = results.filter(r => r.status === 'rejected').length;
        if (failed > 0) {
            console.warn(`⚠️  ${failed} restaurant(s) failed processing during nightly job.`);
        }
...
const processSingleRestaurant = async (userId) => {
    try {
        ...
        console.log(`✨ Processed ${findings.length} insights for restaurant ${userId}`);
